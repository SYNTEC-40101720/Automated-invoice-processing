...
重构前的桌面界面同时承担渲染、线程池调度、任务状态、邮箱轮询、目录监听、归档和审核编排，导致状态同步、测试和界面迭代越来越困难。当前实现已将这些职责拆分到应用层、API、Web 工作台和桌面桥接层。
...
密钥字段采用写入专用语义：读取时只返回 `configured: true/false`；更新时空值代表不变，显式 `clear_secret: true` 才清除，杜绝前端把掩码当密钥写回。
...
- 日志初始快照通过 HTTP 获取，WebSocket 只推增量，避免重连时发送无限历史。
...
| 层级 | 工具 | 必测内容 |
|---|---|---|
| 核心回归 | pytest | 保留所有现有核心、邮箱、审核测试 |
| 应用层 | pytest + fake event bus/filesystem | 状态迁移、单任务互斥、取消、归档条件、事件顺序 |
| API | FastAPI TestClient/httpx | DTO 校验、错误码、密钥脱敏、冲突与路径拒绝 |
| WebSocket | pytest | 初始快照、事件顺序、断线重连校准、慢客户端策略 |
| 前端单测 | Vitest + Testing Library | store、按钮状态、设置校验、事件归并 |
| E2E | Playwright | 手动选择到完成、停止、断线恢复、设置、日志过滤 |
| 打包冒烟 | Windows 干净机/域控机 | 启动、WebView2、DPI、中文 PDF、输出打开、退出回收 |
...
验收：目标域控机可启动；WebSocket 通信正常；目录选择、打开目录、中文路径、DPI 和关闭回收正常；确认拖拽能力边界。未通过时只替换桌面壳，不进入业务迁移。
...
验收：前端单测和 Playwright 流程通过；桌面与窄窗口无重叠；125%/150%/175% 缩放可用；断线恢复后状态一致。

### 阶段 4：桌面集成与并行试运行

交付：新 `main.py`、pywebview Bridge、构建脚本，并完成新架构的业务结果对照验收。

验收：同一批脱敏样本经新架构处理后，输出文件集合、税号异常、合并页数、Excel 和审核报告符合既有基准；连续运行与退出无残留端口或后台线程。

### 阶段 5：切换与清理

完成：新入口成为默认，旧 UI 包和 Qt 依赖已删除，README、PROJECT_DEV、打包和运维记录已更新。

验收：完整测试、打包合规、干净机冒烟和用户验收全部通过；回退包已归档且配置格式保持兼容。
