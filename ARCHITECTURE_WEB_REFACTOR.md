...
> 当前仓库以现有源码为准：`infrastructure/`、`tests/e2e/` 和独立的 `scheduler.py` 尚未建立；配置、邮箱轮询和日志持久化分别由 `config_manager.py`、`application/email_poller.py` 和现有日志模块承担。上图保留为后续分层演进目标。
...
1. `main.py` 初始化日志；单实例锁尚未实现，多个实例互斥列为后续项；
...
| 层级 | 工具 | 必测内容 | v7.0 状态 |
|---|---|---|---|
| 核心回归 | pytest | 保留所有现有核心、邮箱、审核测试 | 已通过，119 条 |
...
| 前端单测 | Vitest + Testing Library | store、按钮状态、设置校验、事件归并 | 尚未建立 |
| E2E | Playwright | 手动选择到完成、停止、断线恢复、设置、日志过滤 | 尚未建立 |
| 打包冒烟 | Windows 干净机/域控机 | 启动、WebView2、DPI、中文 PDF、输出打开、退出回收 | 本机发布包启动冒烟已通过，目标机待验 |
...
交付：最小 FastAPI + React + pywebview 打包程序。

验收：本机发布包启动冒烟和服务端契约已通过；目标域控机启动、WebView2、WebSocket、中文路径、DPI 和关闭回收仍待实机验收。拖拽能力边界已有原生选择目录作为正式入口。

### 阶段 1：抽取应用层

交付：`JobService`、`InvoiceFileService`、领域模型、事件总线及单元测试；暂不接 Web UI。

验收：用 CLI/测试可跑完整目录流程；状态机、停止、进度、审核和归档均不依赖 Qt；现有测试全绿。这是后续工作的关键门槛。

### 阶段 2：建立本地 API

交付：FastAPI 路由、Pydantic DTO、统一错误码、WebSocket 和安全令牌。

验收：API 契约测试覆盖启动、停止、重连快照、设置脱敏、邮箱拉取和冲突处理；API 路由不直接调用核心私有方法。

### 阶段 3：实现工作台前端

交付：Activity Bar、Side Bar、Editor Area、Bottom Panel、Status Bar，以及处理、日志、审核、收件箱和设置视图。

验收：前端生产构建和类型检查已通过；Vitest、Playwright、缩放和真实浏览器断线恢复仍待补齐。

### 阶段 4：桌面集成与并行试运行

交付：新 `main.py`、pywebview Bridge 和构建脚本已完成；目标机业务结果对照与连续运行验收待执行。

验收：同一批脱敏样本经新架构处理后，输出文件集合、税号异常、合并页数、Excel 和审核报告符合既有基准；连续运行与退出无残留端口或后台线程。

### 阶段 5：切换与清理

部分完成：新入口成为默认，旧 UI 包和 Qt 依赖已删除，README、PROJECT_DEV 和打包说明已更新；单实例锁、目录监听、前端单测、E2E 和干净机验收仍未完成。

验收：Python/前端构建、Ruff、本机打包合规和本机发布包启动冒烟已通过；干净机冒烟、目标域控验收、用户验收和回退包归档仍待执行。

## 13. 开发任务拆分建议

后续执行者应按依赖顺序领取，禁止前端先复制旧窗口中的业务编排：

| 工作包 | 内容 | 依赖 |
|---|---|---|
| WP-00 | pywebview/WebView2/PyInstaller 技术探针 | 无 |
| WP-01 | Job 模型、状态机、事件模型 | WP-00 |
| WP-02 | 单文件处理与完整 JobService 抽取 | WP-01 |
| WP-03 | 审核、邮箱、调度与归档服务 | WP-02 |
| WP-04 | FastAPI、错误模型、令牌与 WebSocket | WP-02 |
| WP-05 | OpenAPI 类型生成与前端 API 客户端 | WP-04 |
| WP-06 | VS Code 式工作台骨架与主题 token | WP-00 |
| WP-07 | 处理/日志/审核/设置功能视图 | WP-05、WP-06 |
| WP-08 | Native Bridge 与桌面生命周期 | WP-04、WP-07 |
| WP-09 | E2E、差异对比、打包与域控验收 | 全部 |

每个工作包都必须包含实现、对应测试、接口或文档更新，不接受只实现 UI 后再集中补测试。

## 14. 主要风险与控制

| 风险 | 影响 | 控制措施 |
|---|---|---|
| 目标机缺失/禁用 WebView2 | 程序无法显示 | 阶段 0 实机验证；保留 Qt WebEngine 壳层备选 |
...
| 双 UI 长期共存 | 维护成本翻倍 | 只允许阶段 4 临时共存，阶段 5 明确删除旧 UI |
| 包体与启动时间增长 | 域控部署困难 | 使用系统 WebView2，不引入 Electron，不加载 CDN |

## 15. 架构完成定义

截至 2026-08-23，核心业务、应用层、API、Web 工作台、邮箱轮询、配置热加载、日志恢复和本机 PyInstaller 打包已完成并通过自动化验证；以下完成定义中涉及真实浏览器、干净域控环境和未建立的前端测试套件部分，仍不能标记为完成。

只有同时满足以下条件，重构才算完成：

- 旧 UI 中不存在尚未迁移的业务编排；
- Python 核心、应用层、API 和前端边界符合依赖规则；
- HTTP/OpenAPI 与 WebSocket 事件契约已有自动化测试；
- UI 重连可恢复当前任务，停止和关闭行为可预测；
- 业务规则不可回归清单全部通过自动化或样本对比验证；
- EXE 在目标域控环境离线可运行，密钥、端口和日志符合安全要求；
- README、PROJECT_DEV、打包说明和故障排查 SOP 已同步更新。
